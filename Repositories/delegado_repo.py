...
class delegado_repo:
    ...
    def buscar_por_usuario_id(usuario_id):
        conn = delegado_repo.get_db_connection()
        row = conn.execute(
            '''SELECT usuario.nombre, empresa.razonSocial FROM usuario JOIN delegado
                ON usuario.Id = delegado.Id JOIN empresa 
                ON delegado.empresaId = empresa.empresaId 
                WHERE delegado.Id = ?''', (usuario_id,)
        ).fetchone()
        conn.close()
        return dict(row) if row else None
    ...
    def ya_es_delegado(usuario_id):
        conn = delegado_repo.get_db_connection()
        row = conn.execute(
            "SELECT nombre, tipoUsuario FROM delegado WHERE Id = ?", (usuario_id,)
        ).fetchone()
        conn.close()
